...
/**
 * @title USDCVaultV2
 * @notice Basic time-locked stablecoin vault. Each recipient may hold up to
 *         5 active vaults. Early withdrawals incur a flat 10% penalty on the
 *         locked amount; penalties accrue to the contract owner.
 * @dev    Superseded by ArcStashV3, which replaces the flat penalty with a
 *         time-scaled penalty (2% -> 0.5% -> 0.1%). Kept for reference.
 */
contract USDCVaultV2 {
    IERC20 public token;
    address public owner;
    uint256 public constant MAX_VAULTS = 5;
    uint256 public constant PENALTY_BPS = 1000; // flat 10%
    ...
    function earlyWithdraw(uint256 _vaultId) external {
        ...
        uint256 penalty = (vault.amount * PENALTY_BPS) / 10000; // flat 10%
